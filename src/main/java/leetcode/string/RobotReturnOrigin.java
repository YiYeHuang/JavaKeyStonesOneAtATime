package leetcode.string;

import leetcode.tag.level.Easy;
import leetcode.tag.type.StackTag;
import leetcode.tag.type.StringTag;

import java.util.Stack;

/*
657. Robot Return to Origin

There is a robot starting at position (0, 0), the origin, on a 2D plane.
Given a sequence of its moves, judge if this robot ends up at (0, 0) after it completes its moves.

The move sequence is represented by a string, and the character moves[i] represents its ith move.
Valid moves are R (right), L (left), U (up), and D (down).
If the robot returns to the origin after it finishes all of its moves, return true. Otherwise, return false.

Note: The way that the robot is "facing" is irrelevant. "R" will always make the robot move to the right once,
"L" will always make it move left, etc. Also, assume that the magnitude of the robot's movement is the same for each move.

Example 1:

Input: "UD"
Output: true
Explanation: The robot moves up once, and then down once. All moves have the same magnitude,
so it ended up at the origin where it started. Therefore, we return true.


Example 2:

Input: "LL"
Output: false
Explanation: The robot moves left twice. It ends up two "moves" to the left of the origin.
We return false because it is not at the origin at the end of its moves.
 */

@Easy
@StackTag
@StringTag
public class RobotReturnOrigin {

    public boolean judgeCircle(String moves) {
        if (moves.length() %2 != 0) return false;

        Stack<Character> verMove = new Stack<>();
        Stack<Character> horMove = new Stack<>();

        for (int i = 0; i < moves.length(); i++) {
            char move = moves.charAt(i);

            if (move == 'L') {
                if (!horMove.isEmpty() && horMove.peek() == 'R') {
                    horMove.pop();
                } else {
                    horMove.push(move);
                }
            } else if (move == 'R') {
                if (!horMove.isEmpty() && horMove.peek() == 'L') {
                    horMove.pop();
                } else {
                    horMove.push(move);
                }
            } else if (move == 'U') {
                if (!verMove.isEmpty() && verMove.peek() == 'D') {
                    verMove.pop();
                } else {
                    verMove.push(move);
                }
            } else {
                if (!verMove.isEmpty() && verMove.peek() == 'U') {
                    verMove.pop();
                } else {
                    verMove.push(move);
                }
            }
        }

        return verMove.isEmpty() && horMove.isEmpty();
    }

    public boolean judgeCircle__counter(String moves) {
        int x = 0;
        int y = 0;
        for (char ch : moves.toCharArray()) {
            if (ch == 'U') y++;
            else if (ch == 'D') y--;
            else if (ch == 'R') x++;
            else if (ch == 'L') x--;
        }
        return x == 0 && y == 0;
    }
}
