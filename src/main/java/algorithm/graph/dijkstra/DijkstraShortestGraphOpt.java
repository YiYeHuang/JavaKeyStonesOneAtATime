 ...
 * O(m log(n)) algorithm.
 */
public class DijkstraShortestGraphOpt {
    ...
    private PriorityQueue<int[]> crossingEdges; // edges with one node explored, another unexplored
    ...
    /**
     *
     * Overrides the Comparator of PriorityQueue.
     *
     */
    ...
    public int[] shortestPath(){
        ...
        while (!crossingEdges.isEmpty()){
            ...
            explored.add(toExplore[0]);
            paths[toExplore[0] - 1] = toExplore[1];
            for (int[] edge : vertices.get(toExplore[0] - 1)) {
                if (!explored.contains(edge[0])) {
                    crossingEdges.add(new int[]{edge[0], edge[1] + toExplore[1]});
                }
            }
        }
        return paths;
    ...
    public static void main(String[] args) throws FileNotFoundException{
        ...
        DijkstraShortestGraphOpt g = new DijkstraShortestGraphOpt("resource/DijkstraData.txt");
